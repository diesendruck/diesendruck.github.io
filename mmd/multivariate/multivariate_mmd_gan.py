...
g = generator(z, width=width, depth=depth, activation=activation,
              out_dim=out_dim)
# X-Values.
v = tf.concat([x[:, 0:1], g[:, 0:1]], 0)
VVT = tf.matmul(v, tf.transpose(v))
sqs = tf.reshape(tf.diag_part(VVT), [-1, 1])
sqs_tiled_horiz = tf.tile(sqs, tf.transpose(sqs).get_shape())
exp_object = sqs_tiled_horiz - 2 * VVT + tf.transpose(sqs_tiled_horiz)
# Y-Values.
v_ = tf.concat([x[:, 1:], g[:, 1:]], 0)
VVT_ = tf.matmul(v_, tf.transpose(v_))
sqs_ = tf.reshape(tf.diag_part(VVT_), [-1, 1])
sqs_tiled_horiz_ = tf.tile(sqs_, tf.transpose(sqs_).get_shape())
exp_object_ = sqs_tiled_horiz_ - 2 * VVT_ + tf.transpose(sqs_tiled_horiz_)
K = 0.0
sigma_list = [0.01, 0.05, 0.1]
for sigma in sigma_list:
    gamma = 1.0 / (2 * sigma**2)
    K += tf.exp(-gamma * exp_object)
sigma_list_ = [0.5, 1.0, 2.0]
#for sigma_ in sigma_list_:
#    gamma_ = 1.0 / (2 * sigma_**2)
#    K += tf.exp(-gamma_ * exp_object_)
